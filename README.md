...
It inherits most of funcionality from SBVB (https://github.com/miguelperezenciso/sbvb1) software but 
is much more user friendly and adds numerous new features such as easy selection implementation, plots
 (PCA, GWAS)... 

Its original target is to simulate genomic selection experiments but can be used as well to study the performance of GWAS or,
...
In order to get full control of all BreedSeq functionalities, basic python knowledge is recommended.
...
For instance, no new mutations are generated. We recommend to use real sequence data or high density SNP data as startup.  
### Requirements
See requirements.txt

### Installation

`sudo python3 -m pip install SeqBreed`

Alternatively, you can download the source code and access the modules by using the full path.
...
chipfile   = 'chip2.pos'   # This file contains a subset of snp positions
chip1file  = 'chr1.pos'    # This file contains snp positions from chr 1

# This generates a GFounder class that contains founder genotypes, ploidy and no. of individuals are inferred
...
```gbase = GFounder(vcfFile, snpFile, minMaf=0)```
...
```gfeatures = Genome(snpFile, mapFile=None, XChr=None, YChr=None, MTChr=None, ploidy=gbase.ploidy)```
...
```doEbv(pop, criterion='random', h2=None, X=None, mkrIds=None, yIds=[], nh=2, itrait=0)```
...
```
newPed = ReturnNewPed(pop, nsel, famsize, mating='random', generation=0)
```
